...
% here we look at *subdata_pre_cereb(isub).pert_resp.comp{1}*

% subdata_pre_cereb(isub).pert_resp.peakcomp
% subdata_pre_cereb(isub).pert_resp.tpeak

%pert_resp.comp{1}
% subdata_pre_cereb(isub).pert_resp.comp{1}   % avergage around the peak -
% normalised so dn

% subdata_pre_cereb(isub).pert_resp.peakcomp{1} % find the peak
% 
% 
% conca comp 1 and 2 together
% 
% subdata_pre_cereb(isub).pert_resp.comp{1}
% and subdata_pre_cereb(isub).pert_resp.comp{2}
...
for isub=1:length(subdata_post_cereb)
peakcomp_pre_cereb{isub}.peakcomp= [subdata_pre_cereb(isub).pert_resp.peakcomp{2} ;    subdata_pre_cereb(isub).pert_resp.peakcomp{1}]
timepeak_pre_cereb{isub}.timepeak= [subdata_pre_cereb(isub).pert_resp.tpeak{1} ;    subdata_pre_cereb(isub).pert_resp.tpeak{2}]
peakcomp_post_cereb{isub}.peakcomp= [subdata_post_cereb(isub).pert_resp.peakcomp{2} ;    subdata_post_cereb(isub).pert_resp.peakcomp{1}];
timepeak_post_cereb{isub}.timepeak= [subdata_post_cereb(isub).pert_resp.tpeak{1} ;    subdata_post_cereb(isub).pert_resp.tpeak{2}];
...
subject_mean_pre_cereb_peakcomp(isub)=mean(peakcomp_pre_cereb{isub}.peakcomp);
subject_mean_post_cereb_peakcomp(isub)=mean(peakcomp_post_cereb{isub}.peakcomp);
subject_mean_pre_cereb_timepeak(isub)=mean(timepeak_pre_cereb{isub}.timepeak);
subject_mean_post_cereb_timepeak(isub)=mean(timepeak_post_cereb{isub}.timepeak);
...
for isub=1:7
peakcomp_pre_sham{isub}.peakcomp= [subdata_pre_sham(isub).pert_resp.peakcomp{2} ;   subdata_pre_sham(isub).pert_resp.peakcomp{1}];
timepeak_pre_sham{isub}.timepeak= [subdata_pre_sham(isub).pert_resp.tpeak{1} ;    subdata_pre_sham(isub).pert_resp.tpeak{2}]
peakcomp_post_sham{isub}.peakcomp= [subdata_post_sham(isub).pert_resp.peakcomp{2} ; subdata_post_sham(isub).pert_resp.peakcomp{1}];
timepeak_post_sham{isub}.timepeak= [subdata_post_sham(isub).pert_resp.tpeak{1} ;  subdata_post_sham(isub).pert_resp.tpeak{2}]


subject_mean_pre_sham_peakcomp(isub)=mean(peakcomp_pre_sham{isub}.peakcomp);
subject_mean_post_sham_peakcomp(isub)=mean(peakcomp_post_sham{isub}.peakcomp);
subject_mean_pre_sham_timepeak(isub)=mean(timepeak_pre_sham{isub}.timepeak);
subject_mean_post_sham_timepeak(isub)=mean(timepeak_post_sham{isub}.timepeak);
...
%% group anova - COMP
gp_anovadata_PEAKCOMP=[subject_mean_pre_cereb_peakcomp subject_mean_post_cereb_peakcomp subject_mean_pre_sham_peakcomp subject_mean_post_sham_peakcomp];
gp_anovadata_TIMEPEAK=[subject_mean_pre_cereb_timepeak subject_mean_post_cereb_timepeak subject_mean_pre_sham_timepeak subject_mean_post_sham_timepeak];
gp_anovadata_COMP= [subject_mean_pre_cereb subject_mean_post_cereb subject_mean_pre_sham subject_mean_post_sham];
...
group1=[site];
group2=[condition];
p_COMP = anovan(gp_anovadata_COMP,{group1 group2 },'model','interaction');
p_PEAKCOMP = anovan(gp_anovadata_PEAKCOMP,{group1 group2 },'model','interaction');
p_PEAKTIME = anovan(gp_anovadata_TIMEPEAK,{group1 group2 },'model','interaction');
